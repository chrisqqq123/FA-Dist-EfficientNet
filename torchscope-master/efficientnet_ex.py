...
params = {
    ...
    'efficientnet_ex': (0.8, 0.8, 32, 0.0),
    'efficientnet_exx': (0.8, 0.7, 32, 0.0),
}
...
def _round_repeats(repeats, depth_mult):
    if depth_mult == 1.0:
        return repeats
    return max(int(math.ceil(depth_mult * repeats)), 1)
...
class EfficientNet(nn.Module):
    ...
    LR_REGIME = [1, 25, 0.07, 26, 50, 0.007, 51, 65, 0.0007, 66, 75, 0.00007]
    # LR_REGIME = [1, 25, 0.08, 26, 35, 0.008, 36, 45, 0.0008, 46, 50, 0.00008]
    def __init__(self, width_mult=1.0, depth_mult=1.0, dropout_rate=0.2, num_classes=100, coslinear=True):
        ...
        # self.Conv_last = nn.Conv2d(last_channels,last_channels, 8, bias=False)
        ...
        if coslinear:
            ...
            self.classifier = nn.Sequential(
                # nn.Dropout(dropout_rate),       # test_2
...
def efficientnet_ex(pretrained=False, progress=True, **kwargs):
    return _efficientnet('efficientnet_ex', pretrained, progress, **kwargs)


def efficientnet_exx(pretrained=False, progress=True, **kwargs):
    return _efficientnet('efficientnet_exx', pretrained, progress, **kwargs)
